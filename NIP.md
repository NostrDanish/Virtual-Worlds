...
This document defines the custom Nostr event kinds used by **Virtual Worlds** — the Atlas of the Multiverse.
...
| `lat`    | No       | Y-coordinate on the atlas map (0–2000) |
| `lng`    | No       | X-coordinate on the atlas map (0–3200) |
| `status` | No       | `"pending"` (awaiting approval) or `"approved"` (on map) |
...
When a pending realm accumulates **5 unique upvotes** (unique by pubkey), it is automatically promoted to `"approved"` status and displayed on the atlas map.
